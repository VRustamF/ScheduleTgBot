...
from data.db import BASE_SCHEDULE, Subjects, WeeklySchedules
            ...
            if cell_val and len(cell_val) > 2:
                if col == day_column and cell_val != current_day:
                    ...
                    if current_day not in added_day_in_groups[groups_columns[col]]:
                        add_day(day_name=current_day, group_name=groups_columns[col], faculty=filename[:-5])
...
with open(f"{FILES_PATH}{settings.schedule.final_schedule}", "w", encoding="utf-8") as f:
